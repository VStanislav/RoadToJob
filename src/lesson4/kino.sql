...
(
    ...
    CONSTRAINT fk_seats_halls FOREIGN KEY(hall_id) REFERENCES halls(id) ON DELETE NO ACTION ON UPDATE NO ACTION);
...
INSERT INTO halls(hall_name, comments)
VALUES ("Зал 1", "малый зал - 10 мест"),
...
INSERT INTO tickets(session_id, seat_id, sold_out)
VALUES
...
FROM (SELECT s.id,
             ...
             s.session_date,
             s.session_time,
             m.duration, TIMESTAMP (s.session_date, s.session_time) AS starts_at,
     TIMESTAMPADD(MINUTE, m.duration, TIMESTAMP(s.session_date, s.session_time)) AS ends_at FROM sessions s
  JOIN movies m
ON s.movie_id = m.id) AS t1
    JOIN (
SELECT s.id, s.hall_id, m.title, s.session_date, s.session_time, m.duration, TIMESTAMP (s.session_date, s.session_time) AS starts_at, TIMESTAMPADD(MINUTE, m.duration, TIMESTAMP (s.session_date, s.session_time)) AS ends_at
FROM sessions s
    JOIN movies m
ON s.movie_id = m.id) AS t2
WHERE t1.starts_at BETWEEN t2.starts_at
  AND t2.ends_at
  AND
    t1.id <> t2.id
  AND
...
# Movie_2	2021-08-12	09:45:00	120	Movie_1	2021-08-12	09:00:00	60
# Movie_2	2021-08-12	09:45:00	120	Movie_1	2021-08-12	09:30:00	60
# Movie_4	2021-08-12	20:00:00	60	Movie_4	2021-08-12	19:00:00	60

SET @checked_hall_id = 1;
SELECT *
FROM (
         WITH timestamps_table AS (SELECT s.id,
                                          s.hall_id,
                                          m.title,
                                          s.session_date,
                                          s.session_time,
...
SELECT tt.title,
       tt.starts_at,
       tt.duration,
       LEAD(starts_at, 1) OVER () next_session_starts_at, TIMESTAMPDIFF(MINUTE, ends_at, LEAD(starts_at, 1) OVER ()) minutes_between_sessions,
       CASE
...
SELECT m.title,
       SUM(CASE
               WHEN s.session_time >= "09:00:00" AND s.session_time < "15:00:00"
                   THEN 1
               ELSE 0 END) AS "09_15_viewers",
       SUM(CASE
               WHEN s.session_time >= "09:00:00" AND s.session_time < "15:00:00"
                   THEN p.price
               ELSE 0 END) AS "09_15_fee",
       SUM(CASE
               WHEN s.session_time >= "15:00:00" AND s.session_time < "18:00:00"
                   THEN 1
               ELSE 0 END) AS "15_18_viewers",
       SUM(CASE
               WHEN s.session_time >= "15:00:00" AND s.session_time < "18:00:00"
                   THEN p.price
               ELSE 0 END) AS "15_18_fee",
       SUM(CASE
               WHEN s.session_time >= "18:00:00" AND s.session_time < "21:00:00"
                   THEN 1
               ELSE 0 END) AS "18_21_viewers",
       SUM(CASE
               WHEN s.session_time >= "18:00:00" AND s.session_time < "21:00:00"
                   THEN p.price
               ELSE 0 END) AS "18_21_fee",
       SUM(CASE
               WHEN s.session_time >= "21:00:00" AND
                    s.session_time <= "23:59:59" THEN 1
               ELSE 0 END) AS "21_00_viewers",
       SUM(CASE
               WHEN s.session_time >= "21:00:00" AND
                    s.session_time <= "23:59:59" THEN p.price
               ELSE 0 END) AS "21_00_fee"
FROM tickets t
         JOIN sessions s ON s.id = t.session_id
         JOIN prices p ON s.id = p.session_id
         JOIN movies m ON s.movie_id = m.id
